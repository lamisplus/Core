...
import SmsSetUpCreation from './SmsSetUpCreation';
...
const SmsSetUp = (props) => {
    ...
    const fetchNotificationConfigs = () => {
        axios.get(`${baseUrl}sms-setup/all-setups`, {
    ...
    const openNotificationConfigDeletionModal = (row) => {
        axios
            .delete(`${baseUrl}sms-setup/${row}`, {
                headers: { Authorization: `Bearer ${token}` },
            })
            .then((response) => {
                setModal(false)
                fetchNotificationConfigs()
                toast.success("Sms Setup successfully deleted...")
            })
            .catch(error => {
                console.log(error)
                 }
    ...
    return (
        <>
            ...
            <SmsSetUpCreation toggleModal={toggleModal} showModal={showCreationModal}
            ...
            <Card>
                <CardBody >
                    <div className={"d-flex justify-content-end"}>
                        ...
                        >
                            <span style={{ textTransform: 'capitalize' }}>Add SMS Config</span>
                        </ButtonMui>}
                    </div>
                    <MaterialTable
                        icons={tableIcons}
                        title="Find SMS Setup"
                        columns={[
                            {
                                title: "Sender ID",
                                field: "senderID",
                            },
                            { title: "Message Category", field: "messageCategory" },
                            {
                                title: "Frequency",
                                field: "frequency",
                            },
                        ...
                        isLoading={loading}
                        data={notificationConfigList.map((row, i) => ({
                            id: i + 1,
                    ...
                    <Modal show={modal}>
                        ...
                        <Modal.Header>
                            <Modal.Title>Delete Sms Setup </Modal.Title>
                            <Button
                                variant=""
                                className="btn-close"
                        ...
                        <Modal.Body>
                            <p>Are you sure you want to delete Sms Setup</p>
                        </Modal.Body>
                        <Modal.Footer>
                            <MatButton
                                type="submit"
                                ...
                                onClick={() => openNotificationConfigDeletionModal(rowNum)}
                            >
                                Yes
                            </MatButton>
                            <MatButton
