...
import SendSmsCreation from './SendSmsCreation';
...
const SendSms = (props) => {
    ...
    const fetchNotificationConfigs = () => {
        axios.get(`${baseUrl}sms/sms-history`, {
    ...
    const openNotificationConfigDeletionModal = (row) => {
        axios
            ...
            .then((response) => {
                setModal(false)
                fetchNotificationConfigs()
                toast.success("Sms successfully deleted...")
            })
            .catch(error => {
                console.log(error)
            }
    ...
    return (
        <>
            ...
            <SendSmsCreation toggleModal={toggleModal} showModal={showCreationModal}
            ...
            <Card>
                <CardBody >
                    <div className={"d-flex justify-content-end"}>
                        ...
                        >
                            <span style={{ textTransform: 'capitalize' }}>Send Custom SMS</span>
                        </ButtonMui>}
                    </div>
                    <MaterialTable
                        icons={tableIcons}
                        ...
                        columns={[
                            ...
                            { title: "Sender", field: "senderID" },
                            { title: "Message", field: "message" },
                            { title: "Status", field: "sendStatus" },
                            { title: "Date", field: "timeStamp" },

                            { title: "Action", field: "action" }
                        ]}
                        isLoading={loading}
                        data={notificationConfigList.map((row, index) => ({
                            id: index + 1,
                    ...
                    <Modal show={modal}>
                        ...
                        <Modal.Header>
                            <Modal.Title>Delete Sms </Modal.Title>
                            <Button
                                variant=""
                                className="btn-close"
                        ...
                        <Modal.Body>
                            <p>Are you sure you want to delete Sms</p>
                        </Modal.Body>
                        <Modal.Footer>
                            <MatButton
                                type="submit"
